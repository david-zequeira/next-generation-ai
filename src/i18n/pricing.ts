 ...
 *
 * - El trabajo es el mismo. Mismo alcance y mismo entregable. Gratis no es "más
 ...
 * ---
 *
 * **Corrección del 17/09/2026 — el Diagnóstico tarda 48 HORAS, no dos semanas.**
 *
 * La web decía dos semanas y la ficha del agente (`ng-agent/profiles/nextgen-ai.json`)
 * decía 48 horas, en tres sitios. El lead leía una cifra aquí, le preguntaba al
 * chat y le daban otra — dentro de la misma visita. Manda la ficha: 48 horas es
 * lo que se entrega de verdad, y es lo que el agente lleva contestando.
 *
 * Al tocar este plazo hay que tocar los TRES sitios a la vez: este `diag`, el
 * `diag` de `calculadora.ts`, y la ficha (nombre del servicio, su descripción y
 * las dos FAQs que lo mencionan).
 *
 * ---
 *
 * **17/09/2026 — «Starter» pasa a llamarse «Arranque».**
 *
 * Esta página era el único sitio que lo llamaba Starter. `/contacto`, la ficha
 * del agente, el catálogo de ng-agent, la tarifa y las propuestas que se envían
 * decían Arranque desde siempre: el prospecto leía un nombre aquí y recibía la
 * propuesta de otro, y el desplegable de contacto al que llegaba desde el botón
 * de esta misma tarjeta ya le enseñaba «Arranque».
 *
 * El nombre publicado ES AHORA el slug (`plan.name.toLowerCase()` en
 * `PricingPage.tsx` construye `/contacto?plan=arranque`), así que ya no hay una
 * traducción a mano que pueda desincronizarse del título.
 *
 * **En inglés también se llama Arranque**, igual que Core y Nexus, que tampoco
 * se traducen: son nombres de producto, y tener dos de tres sin traducir era lo
 * que hacía que el tercero cantara. El significado lo lleva la etiqueta
 * («Live in 7 days») y el `who` de la tarjeta, no el nombre.
 *
 * ---
 *
 * **17/09/2026 — multi-sede: se vende lo que se entrega, no lo que se diseñó.**
 *
 * El comparador decía "1" en Arranque y "1 + add-on" en Core, dos planes que por
 * capacidades son idénticos. Ahora los dos dicen "1 + add-on", que es la verdad.
 *
 * Lo importante es lo que se QUITÓ del módulo: *"gestionado desde el mismo
 * panel"*. Ese panel consolidado **no existe** — en ng-agent, `tenantByPanelToken`
 * devuelve UN cliente, así que un dueño con dos sedes recibe dos enlaces, no uno.
 * Tampoco existe el "enrutado automático por sede": no hay concepto de sede ni en
 * el agente ni en las herramientas de reserva, y `google_calendar_id` es un único
 * campo de texto por cliente. Una sede adicional se entrega hoy como un cliente
 * más dado de alta a mano, y el texto lo dice en vez de insinuar lo contrario.
 *
 * Y una consecuencia que no se ve desde aquí: los 49 €/mes de cada sede **no se
 * registran en ninguna parte**. `monthlyRevenue()` solo suma bloques de
 * conversaciones y de voz, así que el MRR del panel de agencia se queda corto por
 * cada sede vendida. Antes de vender la segunda, ver `plans.ts` en ng-agent.
 *
 * ---
 *
...
const es: PricingDict = {
  ...
  plans: [
    {
      ...
      quota: ["1.000 conversaciones/mes", "Web + WhatsApp", "Agente en 2 idiomas", "1 sede (+ add-on)", "2 h/mes de cambios"],
    ...
    },
    {
      ...
      features: [
        ...
        { text: "**SEO técnico y de contenidos**: estructura, velocidad, metadatos y Search Console, para que Google te encuentre" },
        { text: "**Y para que te encuentren las IA**: datos estructurados y contenido que ChatGPT, Perplexity y los resúmenes de Google saben leer y citar" },
      ],
      quota: ["1.000 conversaciones/mes", "Web bilingüe ES/EN", "SEO incluido", "Agente en 2 idiomas", "1 sede (+ add-on)", "2 h/mes de cambios"],
  ...
  diag: {
    ...
    body: "**Diagnóstico de IA — 0 €.** 48 horas. Mapeamos tus procesos, medimos por dónde se te escapan clientes (llamadas sin contestar, mensajes fuera de horario, huecos sin revender) y te entregamos las oportunidades ordenadas por retorno, con las cifras de tu negocio. **Corre de nuestra cuenta**: preferimos enseñarte el número antes de pedirte una decisión. El informe es tuyo, lo contrates o no.",
  },
  compare: {
    eyebrow: "Comparativa",
    title: "Qué incluye cada nivel",
    sub: "El Arranque y el Core montan exactamente el mismo asistente. La diferencia es que el Core además te construye la marca y la web, y trabaja para que te encuentren: en Google y en los buscadores con IA.",
    cols: ["Arranque", "Core", "Nexus"],
    rows: [
      { label: "Cuota mensual", cells: ["349 €", "449 €", "desde 1.900 €"], strong: true },
      { label: "Puesta en marcha", cells: ["2.900 €", "5.900 €", "desde 18.000 €"], strong: true },
      { label: "…o sin entrada, 18 meses", cells: ["510 €/mes", "779 €/mes", "a medida"], accent: true },
      { label: "Plazo de entrega", cells: ["7 días", "2 semanas", "A definir"], strong: true },
      ...
      { label: "SEO para Google", cells: ["—", "✓", "✓"] },
      { label: "Visibilidad en buscadores con IA", cells: ["—", "✓", "✓"] },
      { label: "Idiomas del agente", cells: ["2", "2", "Ilimitados"] },
      { label: "Sedes", cells: ["1 + add-on", "1 + add-on", "Varias"] },
  ...
  addons: {
    ...
    items: [
      { name: "La web, después", desc: "Empezaste por el Arranque y ahora quieres la marca y la web premium. Se añade sin tocar el asistente.", price: "3.000 €" },
      ...
      { name: "Sede adicional", desc: "Otro centro con su propia agenda, su horario, su equipo y sus métricas separadas, y su propio acceso al panel. Lo montamos y lo damos de alta nosotros.", price: "490 €", tail: "+ 49 €/mes" },
      ...
      { name: "Integración con tu CRM", desc: "Conectamos el agente a tu CRM —cualquiera con API abierta— para que cada lead y cada conversación entren donde ya trabajas.", price: "2.900 €", tail: "+ 49 €/mes" },
      { name: "Otras integraciones", desc: "POS, ERP y otros sistemas se estudian y se presupuestan en el Diagnóstico. No vendemos conectores que no existen.", price: "a presupuestar" },
  ...
  pay: {
    ...
    items: [
      ...
      { k: "Sin entrada", title: "0 €", body: "Sin desembolso inicial. La puesta en marcha se reparte en la cuota durante 18 meses y a partir del mes 19 baja sola a la cuota del plan. Lleva permanencia de 18 meses.", foot: "510 €/mes → 349 €/mes" },
      { k: "Lo mismo", title: "±0,1 %", body: "Las dos vías cuestan prácticamente igual: 9.180 € en 18 meses sin entrada, frente a 9.182 € pagándolo al contado. Cambia cuándo pagas, no cuánto.", foot: "Sin recargo por financiar" },
    ],
  },
  faq: {
    ...
    items: [
      ...
      { q: "¿Hay permanencia? ¿Puedo irme?", a: "Si pagas la puesta en marcha por adelantado, lo construido es tuyo y la cuota es mensual, sin permanencia: te vas cuando quieras. Si entras con 0 € de entrada, la permanencia es de 18 meses — que es el tiempo en el que se termina de pagar el trabajo. En los dos casos, cuando te vas te descargas todos tus datos —conversaciones, reservas, clientes, leads— en un clic. Sin rehenes." },
      ...
      { q: "¿Cuánto tarda en estar funcionando?", a: "El Arranque, **7 días** desde que nos pasas los datos de tu negocio. El Core, **2 semanas**, porque incluye construir la marca y la web. En los dos casos necesitamos unas 2–3 horas de tu tiempo, repartidas en dos sesiones." },
      ...
      { q: "¿Trabajáis fuera de España?", a: "Sí. Trabajamos con clientes en todo el mundo, en remoto. La tarifa se ajusta al mercado local; escríbenos y te pasamos la de tu país." },
    ],
  },
  final: {
    eyebrow: "Siguiente paso",
    titleA: "Treinta minutos y te decimos",
    ...
    mini: "Precios en euros, sin IVA. Vigentes desde septiembre de 2026.",
  },
};
...
const en: PricingDict = {
  ...
  plans: [
    {
      ...
      quota: ["1,000 conversations/mo", "Web + WhatsApp", "Agent in 2 languages", "1 location (+ add-on)", "2 h/mo of changes"],
      cta: "Start here",
    },
    {
      name: "Core",
      ...
      mrr: "€449",
      mrrNote: "/mo · managed service",
      kitline: "3 instalments, no surcharge · or **€0 upfront** at €779/mo for 18 months",
      features: [
        ...
        { text: "**Technical and content SEO**: structure, speed, metadata and Search Console, so Google finds you" },
        { text: "**And so the AIs find you**: structured data and content that ChatGPT, Perplexity and Google's AI summaries can read and cite" },
      ],
      quota: ["1,000 conversations/mo", "Bilingual website EN/ES", "SEO included", "Agent in 2 languages", "1 location (+ add-on)", "2 h/mo of changes"],
  ...
  diag: {
    ...
    body: "**AI Diagnostic — €0.** 48 hours. We map your processes, measure where clients slip away (unanswered calls, after-hours messages, unsold slots) and hand you the opportunities ranked by return, using your business's own numbers. **It's on us**: we'd rather show you the number before asking you for a decision. The report is yours, whether you sign up or not.",
  },
  compare: {
    eyebrow: "Comparison",
    title: "What each level includes",
    sub: "Arranque and Core run exactly the same assistant. The difference is that Core also builds your brand and website, and works to get you found: on Google and in AI search.",
    cols: ["Arranque", "Core", "Nexus"],
    rows: [
      { label: "Monthly fee", cells: ["€349", "€449", "from €1,900"], strong: true },
      { label: "Setup", cells: ["€2,900", "€5,900", "from €18,000"], strong: true },
      { label: "…or €0 upfront, 18 months", cells: ["€510/mo", "€779/mo", "custom"], accent: true },
      { label: "Delivery time", cells: ["7 days", "2 wks", "To be defined"], strong: true },
      ...
      { label: "SEO for Google", cells: ["—", "✓", "✓"] },
      { label: "Visibility in AI search", cells: ["—", "✓", "✓"] },
      { label: "Agent languages", cells: ["2", "2", "Unlimited"] },
      { label: "Locations", cells: ["1 + add-on", "1 + add-on", "Several"] },
  ...
  addons: {
    ...
    items: [
      { name: "The website, later", desc: "You started with Arranque and now you want the brand and the premium website. It's added without touching the assistant.", price: "€3,000" },
      ...
      { name: "Additional location", desc: "Another site with its own calendar, hours, team and separate metrics, and its own access to the dashboard. We build it and set it up for you.", price: "€490", tail: "+ €49/mo" },
      ...
      { name: "CRM integration", desc: "We connect the agent to your CRM — any with an open API — so every lead and conversation lands where you already work.", price: "€2,900", tail: "+ €49/mo" },
      { name: "Other integrations", desc: "POS, ERP and other systems get assessed and quoted in the Diagnostic. We don't sell connectors that don't exist.", price: "quoted" },
      ...
      { name: "Content & training", desc: "Brand photography and video, plus €490 team training so you get everything out of the system.", price: "from €690" },
    ],
  },
  pay: {
    eyebrow: "Ways to pay",
    ...
    items: [
      ...
      { k: "Instalments", title: "3", body: "40% on signing, 30% when the assistant starts answering and 30% once it's been booking for a week. No surcharge, no interest.", foot: "€1,160 / €870 / €870" },
      { k: "Nothing upfront", title: "€0", body: "No initial payment. The setup is spread across the monthly fee over 18 months and from month 19 it drops on its own to the plan's fee. It carries an 18-month commitment.", foot: "€510/mo → €349/mo" },
  ...
  faq: {
    ...
    items: [
      ...
      { q: "I already have a website. Can I get just the assistant?", a: "Yes, and that's exactly **Arranque**: €2,900 setup and €349/mo. The assistant installs on the website you already have and on your WhatsApp, with nothing rebuilt. It's running in **7 days**." },
      { q: "I start with Arranque. What if I want the website later?", a: "It can be added whenever you like for **€3,000**, the exact setup difference with Core, and the fee goes from €349 to **€449/mo**: the website and its ranking need maintaining too. What's already running doesn't get rebuilt: the assistant, your conversation history and your bookings stay as they are, and the website gets built around them." },
      ...
      { q: "How long until it's up and running?", a: "Arranque, **7 days** from the moment you send us your business's details. Core, **2 weeks**, because it includes building the brand and the website. Either way we need about 2–3 hours of your time, split into two sessions." },
      ...
      { q: "Do you work outside Spain?", a: "Yes. We work with clients all over the world, remotely. Pricing adjusts to the local market; write to us and we'll send you your country's rates." },
    ],
  },
  final: {
    eyebrow: "Next step",
    titleA: "Thirty minutes and we'll tell you",
    ...
    cta: "Book the call",
    mini: "Prices in euros, VAT not included. Valid from September 2026.",
  },
};
